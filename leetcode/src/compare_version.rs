...
fn compare_version1(version1: String, version2: String) -> i32 {
    ...
    for i in 0..len {
        let left = v1.get(i).unwrap_or(&0);
        let right = v2.get(i).unwrap_or(&0);
        if left > right {
            return 1;
        };
        if left < right {
...
fn test_compare_version() {
    let result = compare_version(String::from("7.5.2.4"), String::from("7.5.3.5.6"));
    println!("{}", result);
    let result = compare_version1(String::from("7.5.2.4"), String::from("7.5.3.5.6"));
    println!("{}", result)
}
